...
# Uses a four time horizons, decision_step=5 and influx=0.1.
...
function sensitivity()
    ...
    scenarios = [
        ...
        Dict(:name => "2. Allow restrictive option", :decision_step => 5.0,
             :minimum_influx => 0.02, :maximum_influx => 0.3),
        Dict(:name => "3. Favour lower influx", :decision_step => 5.0, :method => "favor_positive"),
        Dict(:name => "4. Minimal change", :decision_step => 5.0, :method => "closer_more_likely"),
        Dict(:name => "5. Reactive decisions", :decision_step => 5.0, :method => "state_change"),
